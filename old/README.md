...
- Applies a cluster-level expression threshold so low-expression markers do not drive annotation
...
- `--expression-threshold` controls the minimum cluster-average expression a positive marker must reach before it contributes to the score. For negative markers, the scoring rewards clusters that stay below that threshold.
